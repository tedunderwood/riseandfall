...
cutoff = np.quantile(meta['Topic_Perc_Contrib'], .0)

allgenres = []

genrestouse = pd.read_csv('../genre_80_mallet_categories_for_20c_exp.csv')
for idx, row in genrestouse.iterrows():
    if not pd.isnull(row.possible_genres) and len(row.possible_genres.strip()) > 1:
        allgenres.append(str(row.Topic_Num))

allvolsingenrect = 0
greaterthanmedianct = 0

for g in allgenres:
    df = meta.loc[meta['Dominant_Topic'] == g, : ]
    allvolsingenrect += len(df)
    # df = df.loc[meta['Topic_Perc_Contrib'] >= cutoff, : ]
    greaterthanmedianct += len(df)
...
allnonrandom = set()
for g in allgenres:
    allnonrandom = allnonrandom.union(set(genredocs[g]))
...
rand_df = pd.read_csv('random_metadata_4kirkus.tsv', sep = '\t', index_col = 'docid')
...
havevectors = set()
errors = 0
with open('delta_matrix_4kirkus.tsv', encoding = 'utf-8') as f:
    for line in f:
...
with open('kirkus_delta_results_uncut.tsv', mode = 'w', encoding = 'utf-8') as f:
